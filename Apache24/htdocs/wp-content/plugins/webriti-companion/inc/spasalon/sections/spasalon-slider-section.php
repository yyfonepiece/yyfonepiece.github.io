...
{
$slider_thumbnail_two = isset($current_options['slider_thumbnail_two'])? get_the_post_thumbnail_url($current_options['slider_thumbnail_two']) : WC__PLUGIN_URL .'/inc/spasalon/images/slider/home_thumb.jpg';
}else{$slider_thumbnail_two = isset($current_options['slider_thumbnail_two'])? $current_options['slider_thumbnail_two'] : WC__PLUGIN_URL .'/inc/spasalon/images/slider/home_thumb.jpg';
}

}else{
	$slider_thumbnail_two = WC__PLUGIN_URL .'/inc/spasalon/images/slider/home_thumb.jpg';
}
if(isset($current_options['slider_thumbnail_three'])){
if(is_numeric ($current_options['slider_thumbnail_three']))
{
$slider_thumbnail_three = isset($current_options['slider_thumbnail_three'])? get_the_post_thumbnail_url($current_options['slider_thumbnail_three']) : WC__PLUGIN_URL .'/inc/spasalon/images/slider/home_thumb.jpg';
}else{$slider_thumbnail_three = isset($current_options['slider_thumbnail_three'])? $current_options['slider_thumbnail_three'] : WC__PLUGIN_URL .'/inc/spasalon/images/slider/home_thumb.jpg';
}
}else{
	$slider_thumbnail_three = WC__PLUGIN_URL .'/inc/spasalon/images/slider/home_thumb.jpg';
}
		...
		$line_one    = isset($current_options['line_one'])? $current_options['line_one'] : esc_html__('Producing','webriti-companion');
		$line_two    = isset($current_options['line_two'])? $current_options['line_two'] : esc_html__('Experience','webriti-companion');
		...
		$call_us_text    = isset($current_options['call_us_text'])? $current_options['call_us_text'] : esc_html__('Call us on','webriti-companion');
		$call_us    = isset($current_options['call_us'])? $current_options['call_us'] : esc_html__('201 567 89785','webriti-companion');
	...
	<section class="slider">
		<div id="slider">
			<ul class="slide-img slides">
					<?php if($slider_post) { ?>
					<img alt="slide1" src="<?php echo $slider_post; ?>"/>	
					<?php } if( $current_theme_content['slider_bannerstrip_enable'] == true ) { ?>
					<div class="container topbar-detail">
						<div class="col-md-3">
							<div class="title">
								<?php if($line_one) { ?>
								<h4><?php echo $line_one ;  ?></h4> <?php } if($line_two) { ?>
								<h1><?php echo $line_two; ?></h1>
								<?php } ?>
							</div>
						</div>
						<div class="col-md-6">
						...
						<?php  if($call_us_text) { echo esc_attr($call_us_text); }  if($call_us) { ?> <strong><?php echo esc_attr($call_us); ?></strong> <?php } ?></address></div></div>
					</div>
				...
				<div id="carousel">
					<div class="slider-thumb-container container padding-none">
						<div class="one-slide-thumb thumb-img-container">
							...
							<img src="<?php echo $slider_thumbnail_one; ?>"  class="slider-thumb" />
							<?php } ?>
						</div>
						<div class="two-slide-thumb thumb-img-container">
							<?php 	
							 if($slider_thumbnail_two) {  ?>
							<img src="<?php echo $slider_thumbnail_two; ?>"  class="slider-thumb" />
							<?php } ?>
						</div>
						<div class="three-slide-thumb thumb-img-container">
							<?php 	
							 if($slider_thumbnail_three) {  ?>
							<img src="<?php echo $slider_thumbnail_three; ?>"  class="slider-thumb" />
							<?php } ?>
						</div>
					</div>				
				</div>
